...
window = Tk()
window.title("Travel agency management system")
...
e3 = Entry(window, textvariable=destination_text)
...
def search_command():
    clear_list()
    passengers = backend.search(name_text.get(), phone_text.get(), phone_text.get(), age_text.get())
    insert_list(passengers)
...
def add_command():
    backend.insert(name_text.get(), phone_text.get(), destination_text.get(), age_text.get())
    view_command()
...
def update_command():
    backend.update(selected_passenger[0], name_text.get(), phone_text.get(), destination_text.get(), age_text.get())
    view_command()
